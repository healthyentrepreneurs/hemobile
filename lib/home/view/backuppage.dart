...
class _BackupPageState extends State<BackupPage>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<DatabaseBloc, DatabaseState>(
      builder: (context, state) {
        ...
        bool isUploadingData = context
            .select((DatabaseBloc bloc) => bloc.state.isUploadingData ?? false);
        double uploadProgress = context
            .select((DatabaseBloc bloc) => bloc.state.uploadProgress ?? 0.0);
        debugPrint(
  ...
  _uploadData_temp() async {
    ...
    final uploadProgress = currentState.uploadProgress;
